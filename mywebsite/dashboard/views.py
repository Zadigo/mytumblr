...
class IndexView(LoginRequiredMixin, generic.View):
    def get(self, request, *args, **kwargs):
        context = {
            # 'carts_without_orders': models.Video.statistics.carts_without_orders(),
...
class UpdateProductView(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        images = models.Video.objects.all()
            ...
            if method in authorized_methods:
                ...
                if not name:
                    ...
                    if method == 'from-url':
                        ...
                        models.Video.objects.create(**{'name': name, 'url': url, 'variant': variant})
                        message = {
...
def activate_coupon(request, **kwargs):
    state = False
    coupon = models.Video.objects.get(id=kwargs['pk'])
    if coupon:
        if coupon.active:
            coupon.active = False
        else:   
...
def download_csv(request):
    ...
    if request.GET:
        if method == 'all':
            products = models.Video.product_manager.filter(active=True)
            ...
            if not name:
                ...
                try:
                    products = models.Video.collection_manager.active_products(name)
                except:
                    messages.error(request, "Collection non reconnue - EXP-C3", extra_tags='alert-warning')
                    return http.JsonResponse(data={'state': False, 'code': 'EXP-C3'}, code=200)
        ...
        if method == "current":
            ...
            try:
                ...
                if len(product_ids) == 0:
                    raise http.Http404()
            products = models.Video.objects.filter(pk__in=product_ids)
...
def table_actions(request, **kwargs):
    ...
    products = models.Video.objects.filter(id__in=keys)
    ...
    if method == 'duplicate':
        ...
        models.Video.objects.bulk_create(new_items)
...
def delete_item_via_table(request, **kwargs):
    ...
    if method == 'products':
        item = get_object_or_404(models.Video, id=kwargs['pk'])
...
def duplicate_view(request, **kwargs):
    state = False
    try:
        product = models.Video.objects.get(id=kwargs['pk'])
    except:
        messages.error(
    ...
    try:
        with atomic_transactions.atomic():
            new_product = models.Video.objects.create(**base)
    except:
        messages.error(
...
def create_images(request, **kwargs):
    ...
    if method == 'new':
        ...
        for index, url in enumerate(images):
            if index == 0:
                images_objects.append(models.Video(url=url, main_image=True))
            else:
                images_objects.append(models.Video(url=url))

        new_images = models.Video.objects.bulk_create(images_objects)
    ...
    if method == 'update':
        ...
        product = get_object_or_404(models.Video, id=product_id)
...
def associate_images(request, **kwargs):
    ...
    if images:
        ...
        db_images = models.Video.objects.filter(id__in=images)
...
def unlink_image_on_product_page(request, **kwargs):
    ...
    product = models.Video.objects.get(id=product_id)
    image = models.Video.objects.get(id=image)
